  ...
  return (
    <Modal open={Boolean(note)} onClose={busy ? () => undefined : onClose} title={note?.title ?? ''} eyebrow="ANOTAÇÃO" size="large">
      {note && <div className="details-content">
        <div className="details-meta"><span className={`status-pill ${note.status}`}>{label}</span>{note.addToCalendar && <span className="agenda-badge">□ Na agenda</span>}</div>
