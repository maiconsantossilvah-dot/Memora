  ...
  return (
    <article className={`note-card status-${note.status}`} onClick={onClick} tabIndex={0} role="button" onKeyDown={(event) => (event.key === 'Enter' || event.key === ' ') && onClick()} aria-label={`Abrir anotação ${note.title}`}>
      ...
      <div className="note-card-body">
        <div className="note-card-top"><span className={`status-pill ${note.status}`}>{note.status === 'completed' && <Check size={11} aria-hidden="true" />}{statusLabel[note.status]}</span>{note.addToCalendar && <span className="calendar-mark" title="Na agenda"><CalendarDays size={15} aria-hidden="true" /></span>}</div>
        ...
        <footer><span>{format(parseISO(note.date), "d 'de' MMM", { locale: ptBR })}{note.time ? ` · ${note.time}` : ''}</span><ArrowRight size={14} aria-hidden="true" /></footer>
      </div>
    </article>
  );
}
