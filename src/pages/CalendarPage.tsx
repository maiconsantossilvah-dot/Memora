...
export function CalendarPage({ notes, onOpen }: { notes: Note[]; onOpen: (note: Note) => void }) {
  ...
  return <>
    <div className="hero-row calendar-hero"><div><p className="eyebrow">PLANEJAMENTO</p><h1>Agenda</h1><p className="subtitle">Anotações marcadas para acompanhar no tempo.</p></div><div className="month-controls"><button aria-label="Mês anterior" onClick={() => setMonth(subMonths(month, 1))}>←</button><strong>{format(month, 'MMMM yyyy', { locale: ptBR })}</strong><button aria-label="Próximo mês" onClick={() => setMonth(addMonths(month, 1))}>→</button></div></div>
