...
export function NotesPage({ notes, loading, connection, selectedTag, onSelectedTagChange, onOpen, onCreate, onConfigure }: Props) {
  ...
  return <>
    <div className="hero-row"><div><p className="eyebrow">SEU ESPAÇO PESSOAL</p><h1>Minhas anotações</h1><p className="subtitle">Tudo o que importa, organizado do seu jeito.</p></div><button className="primary-button" onClick={onCreate}>＋ Nova anotação</button></div>
    <div className="toolbar notes-toolbar">
      <label className="search"><span>⌕</span><input aria-label="Pesquisar anotações" placeholder="Pesquisar título, descrição ou tag…" value={search} onChange={(e) => setSearch(e.target.value)} /></label>
    ...
    {connection !== 'connected' ? <section className="welcome-card"><div className="welcome-icon">✦</div><div><p className="eyebrow">PRIMEIROS PASSOS</p><h2>Seu espaço está pronto para ganhar vida.</h2><p>Configure o Firebase para registrar ideias, imagens e compromissos com persistência.</p></div><button className="secondary-button" onClick={onConfigure}>Configurar Firebase <span>→</span></button></section>
